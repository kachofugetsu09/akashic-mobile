...
Native→WebUI 在断线、等待订阅或查看历史时发送 `reply.clear`，绑定当前 session 和 projection generation，只清除临时观察。服务端明确发送的 `reply.status.available=false` 原样保留，不能由原生的空观察合成。这个清除事件属于本地桥，不进入服务端 Message 协议；build 80 的 embedded WebUI 与 snapshot v11 合同一起发布。已有 manifest 兼容检查拒绝 snapshot v10 的旧 OTA 界面并回到 embedded，避免升级 APK 后向旧界面发送未知事件。
