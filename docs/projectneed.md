...
已配对用户必须能在连接校验失败、同步中断或重连期间主动执行“清理缓存并同步”。该入口只清除可从核心重建的服务端投影并保留配对、本地未发送工作、草稿与上传附件；活动 Turn、附件下载或已经开始的重建仍可阻止并发执行。错误连接上的重建先提交本地清理，再通过既有认证重连和 durable cursor 恢复，不要求连接先进入 `READY`。
